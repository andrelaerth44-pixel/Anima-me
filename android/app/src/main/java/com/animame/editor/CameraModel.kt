...
data class AnimatableCameraKey(val frame:Int,val x:Float=0f,val y:Float=0f,val scale:Float=1f,val rotationDeg:Float=0f,val opacity:Float=1f)
data class EvaluatedCamera(val x:Float=0f,val y:Float=0f,val scale:Float=1f,val rotationDeg:Float=0f,val opacity:Float=1f)
enum class CameraInterpolation { STEP, LINEAR, SMOOTH }

/** Composition camera track; separate from the document's CameraKeyframe model. */
class AnimatableCameraTrack(keyframes:List<AnimatableCameraKey> = emptyList(), val interpolation:CameraInterpolation=CameraInterpolation.SMOOTH) {
    private val frames=keyframes.sortedBy{it.frame}.toMutableList()
    fun all():List<AnimatableCameraKey> = frames.toList()
    fun set(k:AnimatableCameraKey){val i=frames.indexOfFirst{it.frame==k.frame};if(i>=0)frames[i]=k else frames.add(k);frames.sortBy{it.frame}}
    ...
    fun evaluate(frame:Float):EvaluatedCamera{
        ...
        return EvaluatedCamera(lerp(a.x,b.x,t),lerp(a.y,b.y,t),lerp(a.scale,b.scale,t).coerceAtLeast(.0001f),lerpAngle(a.rotationDeg,b.rotationDeg,t),lerp(a.opacity,b.opacity,t).coerceIn(0f,1f))
    }
