...
class AnimationEditorViewV4(context: android.content.Context) : AnimationEditorViewV5(context) {
    ...
    override fun onDraw(canvas:Canvas){
        ...
        if(get("panel")?.toString()?.endsWith("BRUSHES")==true) drawBrushLibraryScrolled(canvas)
    }
    ...
    private fun drawBrushLibraryScrolled(c:Canvas){
        val right=width-300f
        val bg=Paint(Paint.ANTI_ALIAS_FLAG).apply{style=Paint.Style.FILL;color=Color.rgb(29,31,34)}
        c.drawRect(right,150f,width.toFloat(),height.toFloat(),bg)
        val clip=RectF(right,204f,width.toFloat(),height.toFloat());c.save();c.clipRect(clip)
        var y=206f-brushScroll
        val text=Paint(Paint.ANTI_ALIAS_FLAG).apply{style=Paint.Style.FILL;textSize=10f}
        for(f in BrushCatalog.families){
            if(y<height+20&&y>180){text.color=Color.WHITE;c.drawText(f,right+14f,y,text)}
            y+=18f
            for(p in BrushCatalog.presets.filter{it.family==f}){
                if(y>185f&&y<height+20){text.color=if(p.id==(get("selectedBrush") as? BrushPreset)?.id)Color.WHITE else Color.LTGRAY;text.textSize=9f;c.drawText(if(p.id==(get("selectedBrush") as? BrushPreset)?.id)"• ${p.name}" else "  ${p.name}",right+20f,y,text)}
                y+=16f
            }
            y+=4f
        }
        c.restore()
        val bar=Paint(Paint.ANTI_ALIAS_FLAG).apply{style=Paint.Style.FILL;color=Color.rgb(53,57,63)};c.drawRoundRect(right,184f,width-10f,204f,4f,4f,bar)
        text.color=Color.WHITE;text.textSize=9f;c.drawText("IMPORTAR QR / IMAGEM",right+10f,198f,text)
        text.color=Color.GRAY;text.textSize=8f;c.drawText("deslize para ver os 900+ pincéis",right+10f,height-10f,text)
    }
    ...
    override fun onTouchEvent(e:MotionEvent):Boolean{
        ...
        if(p.endsWith("BRUSHES") && e.x>=width-300f && e.y>204f){
            when(e.actionMasked){
                MotionEvent.ACTION_DOWN->{brushDragging=true;brushScrollStartY=e.y;brushScrollStartOffset=brushScroll;return true}
                MotionEvent.ACTION_MOVE->{if(brushDragging){brushScroll=(brushScrollStartOffset-(e.y-brushScrollStartY)).coerceIn(0f,brushContentHeight()-260f);invalidate()};return true}
                MotionEvent.ACTION_UP->{if(brushDragging){val moved=abs(e.y-brushScrollStartY);brushDragging=false;if(moved<12f){val b=brushAt(e.y+brushScroll);if(b!=null){set("selectedBrush",b);set("brushSettings",b.defaults.copy());set("brushSize",b.defaults.size);set("opacity",b.defaults.opacity)}}invalidate()};return true}
                MotionEvent.ACTION_CANCEL->{brushDragging=false;return true}
            }
        }
        when(e.actionMasked){
    ...
    private fun brushContentHeight():Float{var y=0f;for(f in BrushCatalog.families){y+=22f;y+=BrushCatalog.presets.count{it.family==f}*16f+4f};return y}
    private fun brushAt(yInput:Float):BrushPreset?{var y=206f;for(f in BrushCatalog.families){y+=18f;for(p in BrushCatalog.presets.filter{it.family==f}){if(yInput in y-16f..y+2f)return p;y+=16f};y+=4f};return null}
    ...
    private fun routeUi(x:Float,y:Float,down:Boolean):Boolean{
        ...
        val right=width-300f;val panel=get("panel")?.toString() ?: ""
        if(panel.endsWith("BRUSHES")&&x>=right&&y in 184f..205f){val activity=context as? android.app.Activity;if(activity!=null)BrushImportBridge.launch(activity){imported->set("selectedBrush",imported.preset);set("brushSettings",imported.preset.defaults.copy());set("brushSize",imported.preset.defaults.size);set("opacity",imported.preset.defaults.opacity);invalidate()};return true}
        if(panel.endsWith("OPTIONS")&&x>=right){if(y in 258f..284f){adjusting=down;adjustKind="size";updateAdjustment(x);return true};if(y in 284f..310f){adjusting=down;adjustKind="opacity";updateAdjustment(x);return true};if(y in 390f..414f){if(down)smoothEnabled=!smoothEnabled;invalidate();return true};if(y in 414f..438f){adjusting=down;adjustKind="stabilizer";updateAdjustment(x);return true};if(y in 438f..464f){if(down)set("antiAlias",!((get("antiAlias") as? Boolean)?:true));invalidate();return true};if(y in 190f..225f){invoke("openColorPopup");return true}}
        if(panel.endsWith("TIMELINE")&&x>=right&&y in 220f..310f){(get("document") as? AnimationDocument)?.let{it.onion.enabled=!it.onion.enabled};invalidate();return true}
        if(panel.endsWith("LAYERS")&&x>=right&&y in 185f..220f){invoke("addLayer");return true};if(panel.endsWith("LAYERS")&&x>=right&&y>220f){invoke("selectLayer",y);return true};if(panel.endsWith("RULERS")&&x>=right&&y>185f){val i=((y-206f)/24f).toInt();if(i in 0..8)set("ruler",i+1);invalidate();return true};if(panel.endsWith("CAMERA")&&x>=right&&y in 285f..340f){invoke("resetCamera");invalidate();return true};return false
    }
