...
class ProjectHomeActivity : Activity() {
    ...
    private fun refreshProjects() {
        ...
        if (projects.length() == 0) {
            list.addView(TextView(this).apply {
                text = "Nenhum projeto ainda.\n\nCrie um novo projeto para começar uma animação."
                textSize = 17f
                setTextColor(Color.LTGRAY)
        ...
        for (i in 0 until projects.length()) {
            ...
            val info = TextView(this).apply {
                text = "${project.optString("name", "Sem nome")}\n${project.optInt("width", 1280)} × ${project.optInt("height", 720)}  |  ${project.optInt("fps", 24)} FPS"
                textSize = 16f
                setTextColor(Color.WHITE)
            }
    ...
    private fun showNewProjectDialog() {
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                val w = width.text.toString().toIntOrNull()?.coerceIn(1, 16384) ?: 1280
                val h = height.text.toString().toIntOrNull()?.coerceIn(1, 16384) ?: 720
                val rate = fps.text.toString().toIntOrNull()?.coerceIn(1, 240) ?: 24
                val initialFrames = frames.text.toString().toIntOrNull()?.coerceIn(1, 10000) ?: 1
                val project = JSONObject().apply {
                    ...
                    put("height", h)
                    put("fps", rate)
                    put("duration", initialFrames)
                    put("transparent", transparent.isChecked)
                }
                saveProject(project)
                dialog.dismiss()
                openProject(project)
    ...
    private fun openProject(project: JSONObject) {
        startActivity(Intent(this, MainActivity::class.java).apply {
            putExtra("project_id", project.optString("id"))
            putExtra("project_name", project.optString("name", "Minha animação"))
            putExtra("project_width", project.optInt("width", 1280))
            putExtra("project_height", project.optInt("height", 720))
            putExtra("project_fps", project.optInt("fps", 24))
            putExtra("project_duration", project.optInt("duration", 1))
            putExtra("project_transparent", project.optBoolean("transparent", false))
        })
    }
    ...
    private fun confirmDelete(id: String) {
        AlertDialog.Builder(this)
            ...
            .setMessage("Excluir este projeto da lista?")
            .setNegativeButton("Cancelar", null)
            .setPositiveButton("Excluir") { _, _ ->
                val old = loadProjects()
                val next = JSONArray()
    ...
    private fun loadProjects(): JSONArray = try {
        JSONArray(getSharedPreferences(prefsName, MODE_PRIVATE).getString(projectsKey, "[]") ?: "[]")
    } catch (_: Exception) { JSONArray() }

    private fun saveProject(project: JSONObject) {
        val old = loadProjects()
        val next = JSONArray().apply { put(project) }
        for (i in 0 until old.length()) put(old.getJSONObject(i))
        getSharedPreferences(prefsName, MODE_PRIVATE).edit().putString(projectsKey, next.toString()).apply()
    }
    ...
    private fun button(label: String, widthDp: Int, action: () -> Unit) = Button(this).apply {
        ...
        layoutParams = LinearLayout.LayoutParams(dp(widthDp), dp(52)).apply { setMargins(dp(4), 0, dp(4), 0) }
    }
