...
object BrushToolState {
    ...
    fun load(context: Context) {
        ...
        size = p.getFloat(SIZE, size).coerceIn(.25f, 4096f)
        opacity = p.getFloat(OPACITY, opacity).coerceIn(0f, 1f)
        flow = p.getFloat(FLOW, flow).coerceIn(0f, 1f)
        spacing = p.getFloat(SPACING, spacing).coerceIn(.005f, 4f)
        smoothing = p.getFloat(SMOOTHING, smoothing).coerceIn(0f, 1f)
