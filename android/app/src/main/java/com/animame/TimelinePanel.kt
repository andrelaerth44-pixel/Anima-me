...
/** Professional frame-by-frame timeline inspired by RoughAnimator's interaction model. */
class TimelinePanel(
    context: Context,
    ...
    init {
        ...
        buildRuler()
        addView(ruler, LayoutParams(-1, 38))
    ...
    private fun refreshRows(accent: Int = Color.rgb(38, 44, 51)) {
        rows.removeAllViews()
        document.layers.forEachIndexed { index, layer -> rows.addView(buildLayerRow(layer, accent, index), LayoutParams(-2, 68)) }
    }
    ...
    private fun buildLayerRow(layer: AnimationLayer, accent: Int, layerIndex: Int): View {
        ...
        val name = EditText(context).apply {
            setText(layer.name); setTextColor(Color.WHITE); textSize = 12f; setSingleLine(true)
            setTypeface(null, if (layer.id == document.selectedLayerId) Typeface.BOLD else Typeface.NORMAL)
            setPadding(2, 0, 2, 0); imeOptions = android.view.inputmethod.EditorInfo.IME_ACTION_DONE
        ...
        bottom.addView(smallButton("Cima", 42) { document.selectLayer(layer.id); document.moveLayer(layer.id, -1); onLayerChanged() })
        bottom.addView(smallButton("Baixo", 42) { document.selectLayer(layer.id); document.moveLayer(layer.id, 1); onLayerChanged() })
        ...
        val opacity = SeekBar(context).apply {
            max = 100; progress = (layer.opacity.coerceIn(0f, 1f) * 100f).toInt()
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(bar: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser) { layer.opacity = value / 100f; onLayerChanged() } }
        ...
        controls.setOnLongClickListener { Toast.makeText(context, "Reordenação: Cima e Baixo alteram a ordem visual", Toast.LENGTH_SHORT).show(); true }
    ...
    private fun commitName(layer: AnimationLayer, value: String) { value.trim().takeIf { it.isNotEmpty() }?.let { layer.name = it }; onLayerChanged() }
    ...
    private fun duplicateLayer(source: AnimationLayer) {
        ...
        document.layers.add(index, copy); document.selectLayer(copy.id); onLayerChanged()
    }

    private fun mergeDown(source: AnimationLayer) {
        val index = document.layers.indexOfFirst { it.id == source.id }
        if (index < 0 || index >= document.layers.lastIndex) { Toast.makeText(context, "Não existe uma camada abaixo para mesclar", Toast.LENGTH_SHORT).show(); return }
        ...
        document.deleteLayer(source.id); document.selectLayer(target.id); onLayerChanged()
    }

    private inner class ExposureTrackView(context: Context, private val layer: AnimationLayer, private val accent: Int) : View(context) {
        ...
        override fun onDraw(c: Canvas) {
            super.onDraw(c)
            val fw = frameWidth().toFloat(); val h = height.toFloat(); var frame = 0
            while (frame < document.duration) {
                val drawing = layer.frames[frame]
                if (drawing == null) { frame++; continue }
                val end = min(document.duration, frame + drawing.exposure.coerceAtLeast(1))
                val left = frame * fw + 2f; val right = end * fw - 2f
                val selected = layer.id == document.selectedLayerId && document.currentFrame in frame until end
                fill.color = when { selected -> accent; drawing.strokes.isNotEmpty() -> Color.rgb(76, 84, 96); else -> Color.rgb(54, 60, 68) }
                c.drawRect(left, 3f, right, h - 3f, fill)
                border.color = if (selected) Color.WHITE else Color.rgb(100, 108, 118); border.style = Paint.Style.STROKE; border.strokeWidth = if (selected) 2f else 1f
                c.drawRect(left, 3f, right, h - 3f, border); border.style = Paint.Style.FILL
                frame = end
            }
            val playheadX = document.currentFrame * fw + fw * .5f
            border.color = Color.WHITE; border.strokeWidth = 2f; c.drawRect(playheadX - 1f, 0f, playheadX + 1f, h, border)
            border.color = Color.argb(80, 255, 255, 255); c.drawRect(document.playbackStart * fw, 0f, (document.playbackEnd + 1) * fw, 2f, border)
        }
        override fun onTouchEvent(event: MotionEvent): Boolean {
            val fw = frameWidth().toFloat()
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    val frame = (event.x / fw).toInt().coerceIn(0, document.duration - 1); val drawing = drawingAtStart(frame)
                    if (drawing != null) {
                        val end = startOf(drawing) + drawing.exposure
                        if (event.x >= end * fw - max(10f, fw * .25f) && event.x <= end * fw + 8f) resizing = drawing else onFrameSelected(layer.id, frame)
                    } else onFrameSelected(layer.id, frame)
                    return true
                }
                MotionEvent.ACTION_MOVE -> {
                    val drawing = resizing ?: return true; val start = startOf(drawing)
                    val newExposure = ((event.x / fw) - start).toInt().coerceIn(1, 120)
                    drawing.exposure = newExposure.coerceAtMost((document.duration - start).coerceAtLeast(1)); onLayerChanged(); invalidate(); return true
                }
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> { resizing = null; invalidate(); return true }
            }
            return true
        }
        private fun drawingAtStart(frame: Int): DrawingFrame? { var best = -1; layer.frames.keys.forEach { key -> if (key <= frame && key > best) best = key }; return if (best >= 0) layer.frames[best] else null }
        private fun startOf(target: DrawingFrame): Int = layer.frames.entries.firstOrNull { it.value === target }?.key ?: 0
    }

    private fun label(text: String, width: Int): TextView = TextView(context).apply { this.text = text; textSize = 8f; gravity = Gravity.CENTER; setTextColor(Color.LTGRAY) }
    private fun smallButton(label: String, width: Int, action: () -> Unit): TextView = TextView(context).apply { text = label; textSize = 8f; gravity = Gravity.CENTER; setTextColor(Color.WHITE); setBackgroundColor(Color.rgb(48, 54, 61)); setOnClickListener { action() }; layoutParams = LinearLayout.LayoutParams(width, 26).apply { setMargins(2, 0, 2, 0) } }
}
