...
class ToolOptionsActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(28, 22, 28, 22); setBackgroundColor(Color.rgb(18, 20, 23)) }
        root.addView(TextView(this).apply { text = "Opções do pincel"; textSize = 24f; setTextColor(Color.WHITE) })
        root.addView(TextView(this).apply { text = "Tamanho, opacidade, flow, espaçamento, suavização e pressão"; textSize = 13f; setTextColor(Color.LTGRAY); setPadding(0, 4, 0, 12) })
        ...
        root.addView(TextView(this).apply { text = "Cor"; textSize = 15f; setTextColor(Color.LTGRAY); setPadding(0, 12, 0, 5) })
        val palette = LinearLayout(this).apply { gravity = Gravity.CENTER; orientation = LinearLayout.HORIZONTAL }
        colors.forEach { color -> palette.addView(Button(this).apply { text = ""; setBackgroundColor(color); setOnClickListener { BrushToolState.color = color; BrushToolState.save(this@ToolOptionsActivity) } }, LinearLayout.LayoutParams(0, 48, 1f).apply { setMargins(3, 2, 3, 2) }) }
        root.addView(palette)
        root.addView(Button(this).apply { text = "Guardar"; setOnClickListener { BrushToolState.save(this@ToolOptionsActivity); setResult(RESULT_OK); finish() } }, LinearLayout.LayoutParams(-1, 52).apply { setMargins(0, 14, 0, 0) })
        setContentView(root)
    }
    ...
    private fun addSlider(root: LinearLayout, title: String, min: Int, max: Int, value: Int, onChanged: (Int) -> Unit) {
        ...
        val bar = SeekBar(this).apply {
            this.max = max - min; progress = (value - min).coerceIn(0, this.max)
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) { val actual = progress + min; label.text = "$title: $actual"; onChanged(actual) }
                override fun onStartTrackingTouch(seekBar: SeekBar) = Unit
                override fun onStopTrackingTouch(seekBar: SeekBar) = Unit
            })
        }
        root.addView(label); root.addView(bar, LinearLayout.LayoutParams(-1, 42))
    }

    private fun check(title: String, checked: Boolean, onChanged: (Boolean) -> Unit) = CheckBox(this).apply { text = title; isChecked = checked; setTextColor(Color.WHITE); setOnCheckedChangeListener { _, value -> onChanged(value) } }
}
