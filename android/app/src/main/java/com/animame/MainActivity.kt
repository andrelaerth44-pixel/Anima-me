    ...
    private inner class EditorSurface : View(this@MainActivity), Choreographer.FrameCallback {
        ...
        override fun onTouchEvent(event:MotionEvent):Boolean{
            ...
            when(event.actionMasked){
                MotionEvent.ACTION_DOWN->{val canvasTop=dp(topBarDp+timelineHeightDp);val canvasLeft=dp(toolBarWidthDp+if(optionsPanel.visibility==View.VISIBLE)optionsWidthDp else 0);if(event.y<canvasTop||event.x<canvasLeft)return false;panActive=false;panPointerId=MotionEvent.INVALID_POINTER_ID;currentSamples.clear();drawing=true;addSample(event);renderPreview();invalidate();return true}
