...
class MainActivity : Activity() {
    ...
    private inner class EditorSurface:View(this@MainActivity),Choreographer.FrameCallback{
        ...
        fun showBrushPicker(){val outer=LinearLayout(this@MainActivity).apply{orientation=LinearLayout.VERTICAL;setPadding(18,8,18,4)};val search=EditText(this@MainActivity).apply{hint="Pesquisar pincel ou categoria"};outer.addView(search);val list=LinearLayout(this@MainActivity).apply{orientation=LinearLayout.VERTICAL};val scroll=ScrollView(this@MainActivity).apply{addView(list)};outer.addView(scroll,LinearLayout.LayoutParams(-1,0,1f));lateinit var dialog:AlertDialog;fun populate(q:String){list.removeAllViews();BrushPresetRepository.search(q).take(80).forEach{p->list.addView(Button(this@MainActivity).apply{text="${p.name}  •  ${p.category}";textSize=12f;setOnClickListener{brushId=p.id;brushName=p.name;brushCategory=p.category;tool=EditorToolEngine.ToolType.BRUSH;Toast.makeText(this@MainActivity,"Pincel: ${p.name}",Toast.LENGTH_SHORT).show();invalidate();dialog.dismiss()}})}};dialog=AlertDialog.Builder(this@MainActivity).setTitle("Pincéis — 389 presets").setView(outer).setNegativeButton("Fechar",null).create();search.addTextChangedListener(SimpleTextWatcher{populate(search.text.toString())});populate("");dialog.show()}
