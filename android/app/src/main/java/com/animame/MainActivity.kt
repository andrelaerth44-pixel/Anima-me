...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        top.addView(button("Camadas", 76) { toggleLayerPanel() })
        ...
        root.addView(controls, FrameLayout.LayoutParams(-1, 58).apply { gravity = Gravity.BOTTOM })

        brushPanel = buildBrushPanel()
        root.addView(brushPanel, FrameLayout.LayoutParams(390, -1).apply {
            gravity = Gravity.END
            topMargin = 66
            bottomMargin = 58
        })
        ...
        root.addView(layerPanel, FrameLayout.LayoutParams(330, -1).apply {
    ...
    private fun buildBrushPanel(): LinearLayout {
        ...
        panel.addView(TextView(this).apply { text = "Tamanho"; setTextColor(Color.LTGRAY); textSize = 12f })
        panel.addView(SeekBar(this).apply {
            max = 4096
            progress = editorBrushSize()
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(s: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser) editor.setBrushSize(value.coerceAtLeast(1).toFloat()) }
                override fun onStartTrackingTouch(s: SeekBar?) = Unit
                override fun onStopTrackingTouch(s: SeekBar?) = Unit
            })
        })
        panel.addView(TextView(this).apply { text = "Opacidade"; setTextColor(Color.LTGRAY); textSize = 12f })
        panel.addView(SeekBar(this).apply {
            max = 100
            progress = (editorBrushOpacity() * 100f).toInt()
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(s: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser) editor.setBrushOpacity((value / 100f).coerceIn(.01f, 1f)) }
                override fun onStartTrackingTouch(s: SeekBar?) = Unit
                override fun onStopTrackingTouch(s: SeekBar?) = Unit
            })
        })
        ...
        panel.addView(android.widget.ScrollView(this).apply { addView(brushList, android.widget.ScrollView.LayoutParams(-1, -2)) }, LinearLayout.LayoutParams(-1, 0, 1f))
    ...
    private fun buildLayerPanel(): LinearLayout {
        ...
        header.addView(button("Nova", 62) { editor.addLayer() })
        header.addView(button("Fechar", 72) { layerPanel.visibility = View.GONE })
        panel.addView(header)

        panel.addView(TextView(this).apply {
            text = "Selecione a camada para desenhar. Visibilidade, bloqueio e opacidade ficam aqui."
            textSize = 11f
            setTextColor(Color.LTGRAY)
            setPadding(4, 4, 4, 8)
        })
        layerList = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        panel.addView(android.widget.ScrollView(this).apply { addView(layerList, android.widget.ScrollView.LayoutParams(-1, -2)) }, LinearLayout.LayoutParams(-1, 0, 1f))
        return panel
    }
    ...
    private fun populateBrushes(category: String) {
        ...
        BrushCatalog.all().filter { it.category == category }.forEach { preset ->
            brushList.addView(Button(this).apply {
                text = preset.name
                textSize = 12f
                setTextColor(Color.WHITE)
                setOnClickListener { editor.selectBrush(preset.id, preset.name); brushPanel.visibility = View.GONE }
            }, LinearLayout.LayoutParams(-1, 48).apply { setMargins(0, 2, 0, 2) })
        }
    }

    private fun populateLayers(layers: List<AnimationLayer>, selectedId: String) {
        ...
        layers.forEachIndexed { index, layer ->
            val row = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
                setPadding(6, 6, 6, 6)
                setBackgroundColor(if (layer.id == selectedId) ThemeColorStore.get(this@MainActivity) else Color.rgb(38, 42, 48))
            }
            val controls = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
            controls.addView(Button(this@MainActivity).apply {
                text = if (layer.visible) "Vis" else "Oculta"
                textSize = 10f
                setOnClickListener { editor.toggleLayerVisibility(layer.id) }
            }, LinearLayout.LayoutParams(55, 44))
            controls.addView(Button(this@MainActivity).apply {
                text = if (layer.locked) "Bloq" else "Livre"
                textSize = 10f
                setOnClickListener { editor.toggleLayerLock(layer.id) }
            }, LinearLayout.LayoutParams(55, 44))
            controls.addView(TextView(this@MainActivity).apply {
                text = layer.name
                textSize = 13f
                setTextColor(Color.WHITE)
                gravity = Gravity.CENTER_VERTICAL
                setOnClickListener { editor.selectLayer(layer.id) }
            }, LinearLayout.LayoutParams(0, 44, 1f))
            controls.addView(Button(this@MainActivity).apply {
                text = "↑"
                textSize = 12f
                setOnClickListener { editor.moveLayer(layer.id, -1) }
            ...
            controls.addView(Button(this@MainActivity).apply {
                text = "X"
                textSize = 12f
                setOnClickListener { editor.deleteLayer(layer.id) }
            }, LinearLayout.LayoutParams(48, 44))
            row.addView(controls)
            row.addView(SeekBar(this@MainActivity).apply {
                max = 100
                progress = (layer.opacity * 100f).toInt()
                setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                    override fun onProgressChanged(s: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser) editor.setLayerOpacity(layer.id, value / 100f) }
                    override fun onStartTrackingTouch(s: SeekBar?) = Unit
                    override fun onStopTrackingTouch(s: SeekBar?) = Unit
                })
            })
            row.setOnClickListener { editor.selectLayer(layer.id) }
            layerList.addView(row, LinearLayout.LayoutParams(-1, 92).apply { setMargins(0, 3, 0, 3) })
        }
    }

    private fun editorBrushSize(): Int = 12
    private fun editorBrushOpacity(): Float = 1f
    private fun toggleBrushPanel() { brushPanel.visibility = if (brushPanel.visibility == View.VISIBLE) View.GONE else View.VISIBLE; if (brushPanel.visibility == View.VISIBLE) populateBrushes(BrushCatalog.categories().firstOrNull() ?: "Simple") }
    private fun toggleLayerPanel() { layerPanel.visibility = if (layerPanel.visibility == View.VISIBLE) View.GONE else View.VISIBLE; if (layerPanel.visibility == View.VISIBLE) editor.refreshLayers() }
    ...
    private inner class EditorSurface : View(this@MainActivity) {
        ...
        override fun onDraw(c: Canvas) {
            ...
            val left = width * .07f; val right = width * .93f; val top = 80f; val bottom = height - 72f
            c.save(); c.translate(panX, panY); c.scale(zoom, zoom)
            c.drawRect(left, top, right, bottom, paper); drawOnionSkin(c); drawDocument(c)
        ...
        private fun drawDocument(c: Canvas) { document.layers.asReversed().forEach { layer -> if (!layer.visible || layer.opacity <= 0f) return@forEach; layer.frameAt(document.currentFrame)?.strokes?.forEach { stroke -> drawStamps(c, BrushEngine.stamps(stroke.samples, stroke.resolvedBrushSettings(), stroke.id.hashCode().toLong()), stroke.color, layer.opacity) } } }
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            ...
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> { if (event.y < 70f || event.y > height - 64f || document.activeLayer.locked) return false; currentSamples.clear(); drawing = true; addSample(event); renderPreview(); invalidate(); return true }
                MotionEvent.ACTION_MOVE -> { if (!drawing) return true; addSample(event); renderPreview(); invalidate(); return true }
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> { if (drawing && event.actionMasked == MotionEvent.ACTION_UP) commitStroke(); drawing = false; currentSamples.clear(); previewStamps = emptyList(); invalidate(); return true }
            }
            return true
        }
        ...
        private fun handleTransform(event: MotionEvent): Boolean {
            when (event.actionMasked) {
                MotionEvent.ACTION_POINTER_DOWN -> { drawing = false; transformMode = true; lastMidX = (event.getX(0) + event.getX(1)) * .5f; lastMidY = (event.getY(0) + event.getY(1)) * .5f; lastDistance = distance(event); currentSamples.clear(); previewStamps = emptyList(); return true }
                MotionEvent.ACTION_MOVE -> { if (!transformMode || event.pointerCount < 2) return true; val midX = (event.getX(0) + event.getX(1)) * .5f; val midY = (event.getY(0) + event.getY(1)) * .5f; panX += midX - lastMidX; panY += midY - lastMidY; val newDistance = distance(event); if (lastDistance > 0f) zoomAround((newDistance / lastDistance).coerceIn(.85f, 1.18f), midX, midY); lastMidX = midX; lastMidY = midY; lastDistance = newDistance; invalidate(); return true }
                MotionEvent.ACTION_POINTER_UP, MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> { transformMode = false; drawing = false; currentSamples.clear(); previewStamps = emptyList(); invalidate(); return true }
            }
            return true
        }
        ...
        private fun zoomAround(factor: Float, x: Float, y: Float) { val old = zoom; zoom = (zoom * factor).coerceIn(.35f, 6f); val scale = zoom / old; panX = x - (x - panX) * scale; panY = y - (y - panY) * scale }
        ...
        private fun addSample(event: MotionEvent) { val p = screenToCanvas(event.x, event.y); val pressure = (if (event.pressure > 1f) event.pressure / 2f else event.pressure).coerceIn(.05f, 1f); currentSamples += StrokeSample(p.first, p.second, pressure, event.eventTime) }
        private fun renderPreview() { val base = if (tool == Tool.ERASER) BrushDefaults.forPreset("eraser") else BrushCatalog.settings(selectedBrushId); previewStamps = BrushEngine.stamps(BrushEngine.smooth(currentSamples, .18f), base.copy(size = brushSize, opacity = brushOpacity), document.currentFrame.toLong()) }
        private fun commitStroke() { if (currentSamples.isEmpty()) return; val frame = document.activeLayer.ensureFrame(document.currentFrame); val color = if (tool == Tool.ERASER) Color.WHITE else accent; val settings = (if (tool == Tool.ERASER) BrushDefaults.forPreset("eraser") else BrushCatalog.settings(selectedBrushId)).copy(size = brushSize, opacity = brushOpacity); frame.strokes += StrokeData(settings.id, color, brushSize, brushOpacity, settings, currentSamples.map { it.copy() }.toMutableList()) }

        fun selectBrush(id: String, name: String) { selectedBrushId = id; brushLabel?.text = name; tool = Tool.BRUSH; invalidate() }
        ...
        fun addLayer() { document.addLayer(); activeLayerId = document.activeLayer.id; invalidate(); refreshTimeline() }
        fun selectLayer(id: String) { if (document.selectLayer(id)) { activeLayerId = id; invalidate(); refreshTimeline() } }
        fun toggleLayerVisibility(id: String) { document.layers.firstOrNull { it.id == id }?.let { it.visible = !it.visible }; invalidate(); refreshLayers(); refreshTimeline() }
        fun toggleLayerLock(id: String) { document.layers.firstOrNull { it.id == id }?.let { it.locked = !it.locked }; invalidate(); refreshLayers(); refreshTimeline() }
        fun setLayerOpacity(id: String, value: Float) { document.layers.firstOrNull { it.id == id }?.opacity = value.coerceIn(0f, 1f); invalidate() }
        fun moveLayer(id: String, delta: Int) { document.moveLayer(id, delta); invalidate(); refreshLayers(); refreshTimeline() }
        fun deleteLayer(id: String) { document.deleteLayer(id); activeLayerId = document.activeLayer.id; invalidate(); refreshLayers(); refreshTimeline() }
        fun refreshLayers() { populateLayers(document.layers.toList(), document.activeLayer.id) }
        ...
        fun clearCurrentFrame() { if (!document.activeLayer.locked) document.activeLayer.frameAt(document.currentFrame)?.strokes?.clear(); invalidate(); refreshTimeline() }

        fun togglePlayback() { if (playing) stopPlayback() else startPlayback() }
        fun startPlayback() { if (document.duration <= 1) { Toast.makeText(this@MainActivity, "Adicione pelo menos 2 frames para reproduzir", Toast.LENGTH_SHORT).show(); return }; playing = true; playbackFrameNanos = System.nanoTime(); playbackAccumulator = 0L; postInvalidateOnAnimation(); schedulePlayback(); refreshTimeline() }
        private fun schedulePlayback() { postDelayed({ playbackTick() }, 8L) }
        private fun playbackTick() { if (!playing) return; val now = System.nanoTime(); playbackAccumulator += (now - playbackFrameNanos).coerceAtLeast(0L); playbackFrameNanos = now; val duration = 1_000_000_000L / document.fps.coerceIn(1, 60); while (playbackAccumulator >= duration) { playbackAccumulator -= duration; document.currentFrame = (document.currentFrame + 1) % document.duration }; invalidate(); refreshTimeline(); schedulePlayback() }
