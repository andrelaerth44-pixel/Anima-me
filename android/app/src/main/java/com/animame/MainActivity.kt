...
class MainActivity : Activity() {
    ...
    private fun buildUi(){
        ...
        val top=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;setPadding(8,6,8,6);setBackgroundColor(ThemeColorStore.NAVY_900)}
        top.addView(button("⚙",50){startActivity(Intent(this@MainActivity,SettingsActivity::class.java))})
        top.addView(button("Pincel",76){editor.selectTool(EditorToolEngine.ToolType.BRUSH)});top.addView(button("Borracha",88){editor.selectTool(EditorToolEngine.ToolType.ERASER)})
        top.addView(button("Balde",68){editor.selectTool(EditorToolEngine.ToolType.BUCKET)});top.addView(button("Laço",60){editor.selectTool(EditorToolEngine.ToolType.LASSO)})
        top.addView(button("Transform",82){editor.selectTool(EditorToolEngine.ToolType.TRANSFORM)});top.addView(button("Conta-gotas",90){editor.selectTool(EditorToolEngine.ToolType.EYEDROPPER)});top.addView(button("Liquify",70){editor.selectTool(EditorToolEngine.ToolType.LIQUIFY)})
        ...
        timelineLabel=TextView(this).apply{setTextColor(ThemeColorStore.TEXT);textSize=13f;setPadding(10,4,10,4)}
        frameStrip=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;setPadding(4,3,4,3)}
        val frameScroll=HorizontalScrollView(this).apply{isHorizontalScrollBarEnabled=false;setBackgroundColor(ThemeColorStore.NAVY_950);addView(frameStrip,FrameLayout.LayoutParams(-2,58))}
        val controls=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;setBackgroundColor(ThemeColorStore.NAVY_900);addView(timelineLabel,LinearLayout.LayoutParams(190,58));addView(button("◀",48){editor.previousFrame()});addView(button("＋",48){editor.insertFrame()});addView(button("⧉",48){editor.duplicateFrame()});addView(button("−",48){editor.deleteFrame()});addView(button("▶",48){editor.nextFrame()});addView(button("Onion",68){editor.toggleOnion()});addView(button("Limpar",70){editor.clearCurrentFrame()});addView(frameScroll,LinearLayout.LayoutParams(0,58,1f))}
    ...
    private fun button(label:String,width:Int,action:()->Unit)=Button(this).apply{text=label;textSize=10f;setTextColor(ThemeColorStore.TEXT);backgroundTintList=ColorStateList.valueOf(ThemeColorStore.NAVY_800);setOnClickListener{action()};layoutParams=LinearLayout.LayoutParams(width,54)}
    ...
    private inner class EditorSurface:View(this@MainActivity){
        ...
        private val toolEngine=EditorToolEngine(); private lateinit var playback: AnimationPlaybackController
        ...
        private var activeLayerId=document.activeLayer.id;private var brushId="canvas_1";private var brushName="Dip Pen (Soft)";private var brushCategory="Simple"
        ...
        var tool=EditorToolEngine.ToolType.BRUSH;var timelineLabel:TextView?=null;var frameStrip:LinearLayout?=null
        private val bg=Paint(Paint.ANTI_ALIAS_FLAG);private val panel=Paint(Paint.ANTI_ALIAS_FLAG);private val paper=Paint(Paint.ANTI_ALIAS_FLAG);private val stampPaint=Paint(Paint.ANTI_ALIAS_FLAG).apply{isDither=true};private val text=Paint(Paint.ANTI_ALIAS_FLAG).apply{color=ThemeColorStore.TEXT;textSize=25f};private val sub=Paint(Paint.ANTI_ALIAS_FLAG).apply{color=ThemeColorStore.MUTED;textSize=15f}
        init{refreshTheme();isFocusable=true;playback=AnimationPlaybackController(document){invalidate();refreshTimeline()}}
        fun selectTool(next:EditorToolEngine.ToolType){tool=next;Toast.makeText(this@MainActivity,"Ferramenta: ${next.name}",Toast.LENGTH_SHORT).show();invalidate()}
        fun refreshTheme(){accent=ThemeColorStore.get(this@MainActivity);bg.color=ThemeColorStore.NAVY_950;panel.color=ThemeColorStore.NAVY_900;paper.color=Color.rgb(248,251,255);text.color=ThemeColorStore.TEXT;sub.color=ThemeColorStore.MUTED}
        override fun onDraw(c:Canvas){c.drawColor(bg.color);val top=66f;val bottom=height-58f;val left=width*.07f;val right=width*.93f;c.drawRect(0f,0f,width.toFloat(),top,panel);val cx=(left+right)*.5f;val cy=(top+bottom)*.5f;c.save();c.clipRect(left,top+14f,right,bottom-14f);c.translate(cx+panX,cy+panY);c.scale(zoom,zoom);c.translate(-cx,-cy);c.drawRect(left,top+14f,right,bottom-14f,paper);drawOnionSkin(c);drawDocument(c);if(previewStamps.isNotEmpty())drawStamps(c,previewStamps,accent,1f);c.restore();c.drawText("ANIMA-ME",78f,38f,text);c.drawText("${tool.name}  •  Frame ${document.currentFrame+1}/${document.duration}  •  ${document.fps} FPS  •  ${document.layers.size} camada(s)",78f,58f,sub);c.drawText("$brushName  •  ${if(playing)"PLAY" else "PAUSE"}  •  ${(zoom*100).toInt()}%",width-330f,38f,sub)}
        ...
        private fun renderPreview(){val base=if(tool==EditorToolEngine.ToolType.ERASER)BrushDefaults.forPreset("eraser")else BrushPresetRepository.find(brushId);previewStamps=BrushEngine.stamps(currentSamples,base.copy(size=brushSize,opacity=brushOpacity,antialias=smoothEdges),document.currentFrame.toLong())}
        private fun commitStroke(){if(currentSamples.isEmpty())return;val frame=document.activeLayer.ensureFrame(document.currentFrame);val color=if(tool==EditorToolEngine.ToolType.ERASER)Color.WHITE else accent;val base=if(tool==EditorToolEngine.ToolType.ERASER)BrushDefaults.forPreset("eraser")else BrushPresetRepository.find(brushId);val settings=base.copy(size=brushSize,opacity=brushOpacity,antialias=smoothEdges);frame.strokes+=StrokeData(brushId=settings.id,color=color,size=brushSize,opacity=brushOpacity,settings=settings,samples=currentSamples.map{it.copy()}.toMutableList())}
        ...
        fun showBrushPicker(){val outer=LinearLayout(this@MainActivity).apply{orientation=LinearLayout.VERTICAL;setPadding(18,8,18,4)};val search=EditText(this@MainActivity).apply{hint="Pesquisar pincel ou categoria"};outer.addView(search);val list=LinearLayout(this@MainActivity).apply{orientation=LinearLayout.VERTICAL};val scroll=ScrollView(this@MainActivity).apply{addView(list)};outer.addView(scroll,LinearLayout.LayoutParams(-1,0,1f));lateinit var dialog:AlertDialog;fun populate(q:String){list.removeAllViews();BrushPresetRepository.search(q).take(80).forEach{p->list.addView(Button(this@MainActivity).apply{text="${p.name}  •  ${p.category}";textSize=12f;setTextColor(ThemeColorStore.TEXT);backgroundTintList=ColorStateList.valueOf(ThemeColorStore.NAVY_800);setOnClickListener{brushId=p.id;brushName=p.name;brushCategory=p.category;tool=EditorToolEngine.ToolType.BRUSH;Toast.makeText(this@MainActivity,"Pincel: ${p.name}",Toast.LENGTH_SHORT).show();invalidate();dialog.dismiss()}})}};dialog=AlertDialog.Builder(this@MainActivity).setTitle("Pincéis — 389 presets").setView(outer).setNegativeButton("Fechar",null).create();search.addTextChangedListener(SimpleTextWatcher{populate(search.text.toString())});populate("");dialog.show()}
        ...
        fun togglePlayback(){if(playing)stopPlayback()else startPlayback()};fun startPlayback(){if(document.duration<=1){Toast.makeText(this@MainActivity,"Adicione pelo menos 2 frames para reproduzir",Toast.LENGTH_SHORT).show();return};playing=true;playback.play();refreshTimeline();invalidate()};fun stopPlayback(){if(!playing)return;playing=false;playback.pause();refreshTimeline();invalidate()}
