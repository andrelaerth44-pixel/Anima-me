...
import com.animame.editor.AnimationPlaybackController
...
import com.animame.editor.ImportedBrushRepository
import com.animame.editor.StrokeData
import com.animame.editor.StrokeSample
import com.animame.editor.StrokeStampCache
...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        val top = LinearLayout(this).apply {
            ...
            setPadding(8, 6, 8, 6)
            setBackgroundColor(ThemeColorStore.NAVY_900)
        }
        top.addView(button("⚙", 50) { startActivity(Intent(this@MainActivity, SettingsActivity::class.java)) })
        top.addView(button("Pincel", 76) { editor.selectTool(EditorToolEngine.ToolType.BRUSH) })
        top.addView(button("Borracha", 88) { editor.selectTool(EditorToolEngine.ToolType.ERASER) })
        top.addView(button("Balde", 68) { editor.selectTool(EditorToolEngine.ToolType.BUCKET) })
        top.addView(button("Laço", 60) { editor.selectTool(EditorToolEngine.ToolType.LASSO) })
        top.addView(button("Transform", 82) { editor.selectTool(EditorToolEngine.ToolType.TRANSFORM) })
        top.addView(button("Conta-gotas", 90) { editor.selectTool(EditorToolEngine.ToolType.EYEDROPPER) })
        top.addView(button("Liquify", 70) { editor.selectTool(EditorToolEngine.ToolType.LIQUIFY) })
        ...
        root.addView(controls, FrameLayout.LayoutParams(-1, 58).apply { gravity = android.view.Gravity.BOTTOM })
        setContentView(root)
    ...
    private fun button(label: String, width: Int, action: () -> Unit) = Button(this).apply {
        ...
        textSize = 10f
        setTextColor(ThemeColorStore.TEXT)
        backgroundTintList = ColorStateList.valueOf(ThemeColorStore.NAVY_800)
    ...
    private inner class EditorSurface : View(this@MainActivity) {
        ...
        private val toolEngine = EditorToolEngine()
        private lateinit var playback: AnimationPlaybackController
        private val rasterCache = EditorRasterCache()
        ...
        private var panX = 0f
        private var panY = 0f
        private var viewportRotation = 0f
        ...
        var tool = EditorToolEngine.ToolType.BRUSH
        ...
        private val stampPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { isDither = true }
        private val text = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = ThemeColorStore.TEXT; textSize = 25f }
        private val sub = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = ThemeColorStore.MUTED; textSize = 15f }

        init {
            refreshTheme()
            isFocusable = true
            playback = AnimationPlaybackController(document) {
                contentVersion++
                invalidate()
                refreshTimeline()
            }
        ...
        fun selectTool(next: EditorToolEngine.ToolType) {
            tool = next
            Toast.makeText(this@MainActivity, "Ferramenta: ${next.name}", Toast.LENGTH_SHORT).show()
            invalidate()
        }

        fun refreshTheme() {
            accent = ThemeColorStore.get(this@MainActivity)
            bg.color = ThemeColorStore.NAVY_950
            panel.color = ThemeColorStore.NAVY_900
            paper.color = Color.rgb(248, 251, 255)
            text.color = ThemeColorStore.TEXT
            sub.color = ThemeColorStore.MUTED
        ...
        override fun onDraw(c: Canvas) {
            ...
            c.rotate(viewportRotation, cx, cy)
            c.scale(zoom, zoom)
            c.translate(-cx, -cy)
            rasterCache.draw(c, width, height, renderVersion()) { cached ->
                cached.drawRect(left, top + 14f, right, bottom - 14f, paper)
            ...
            c.drawText("$brushName  •  ${if (playing) "PLAY" else "PAUSE"}  •  ${(zoom * 100).toInt()}%  •  ${viewportRotation.toInt()}°", width - 390f, 38f, sub)
        }

        private fun renderVersion(): Long = contentVersion xor (document.currentFrame.toLong() shl 32) xor if (onionEnabled) 0L else 1L
        ...
        private fun drawDocument(c: Canvas) {
            document.layers.asReversed().forEach { layer ->
                ...
                frame.strokes.forEach { stroke ->
                    drawStamps(c, stampCache.get(stroke), stroke.color, layer.opacity)
                }
            }
        }
        ...
        private fun drawOnionSkin(c: Canvas) {
            ...
            layer.previousFrames(document.currentFrame, document.onion.previousCount).forEach { (_, frame) -> drawFrameGhost(c, frame, Color.rgb(75, 160, 255), .22f) }
            layer.nextFrames(document.currentFrame, document.onion.nextCount).forEach { (_, frame) -> drawFrameGhost(c, frame, Color.rgb(255, 105, 130), .18f) }
        }

        private fun drawFrameGhost(c: Canvas, frame: DrawingFrame, color: Int, alpha: Float) {
            frame.strokes.forEach { stroke -> drawStamps(c, stampCache.get(stroke), color, alpha) }
        }
        ...
        override fun onTouchEvent(e: MotionEvent): Boolean {
            ...
            when (e.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    if (e.y < 70f || e.y > height - 64f) return false
                ...
                MotionEvent.ACTION_POINTER_DOWN -> {
                    if (e.pointerCount >= 2) {
                        ...
                        pinchStartDistance = pointerDistance(e).coerceAtLeast(1f)
                        pinchStartZoom = zoom
                        pinchStartRotation = viewportRotation
                        lastTouchX = (e.getX(0) + e.getX(1)) * .5f
                        lastTouchY = (e.getY(0) + e.getY(1)) * .5f
                        lastTouchAngle = pointerAngle(e)
                    }
                    return true
                }
                MotionEvent.ACTION_MOVE -> {
                    if (e.pointerCount >= 2) {
                        twoFingerGesture = true
                        val distance = pointerDistance(e).coerceAtLeast(1f)
                        zoom = (pinchStartZoom * distance / pinchStartDistance).coerceIn(.1f, 16f)
                        val mx = (e.getX(0) + e.getX(1)) * .5f
                        val my = (e.getY(0) + e.getY(1)) * .5f
                        panX += mx - lastTouchX
                        panY += my - lastTouchY
                        viewportRotation = normalizeAngle(pinchStartRotation + angleDelta(lastTouchAngle, pointerAngle(e)))
                        lastTouchX = mx
                        lastTouchY = my
                        invalidate()
                        return true
                    }
                    if (twoFingerGesture || !drawing) return true
                    addSample(e)
                    renderPreview()
                    invalidate()
                    return true
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    if (drawing && !twoFingerGesture && e.actionMasked == MotionEvent.ACTION_UP) commitStroke()
                    drawing = false
                    currentSamples.clear()
                    previewStamps = emptyList()
                    invalidate()
        ...
        private fun pointerAngle(e: MotionEvent): Float {
            if (e.pointerCount < 2) return 0f
        ...
        private fun addSample(e: MotionEvent) {
            val pressure = (if (e.pressure > 1f) e.pressure / 2f else e.pressure).coerceIn(.05f, 1f)
            val p = toCanvasPoint(e.x, e.y)
            currentSamples += StrokeSample(p.first, p.second, pressure, e.eventTime)
        }
        ...
        private fun toCanvasPoint(x: Float, y: Float): Pair<Float, Float> {
            ...
            val dx = x - cx - panX
            val dy = y - cy - panY
            val radians = Math.toRadians((-viewportRotation).toDouble())
            val rx = dx * kotlin.math.cos(radians).toFloat() - dy * kotlin.math.sin(radians).toFloat()
            val ry = dx * kotlin.math.sin(radians).toFloat() + dy * kotlin.math.cos(radians).toFloat()
        ...
        fun showBrushPicker() {
            val outer = LinearLayout(this@MainActivity).apply { orientation = LinearLayout.VERTICAL; setPadding(18, 8, 18, 4) }
            ...
            fun populate(q: String) {
                list.removeAllViews()
                BrushPresetRepository.search(q).take(100).forEach { preset ->
                    list.addView(Button(this@MainActivity).apply {
                        ...
                        setOnClickListener {
                            ...
                            tool = EditorToolEngine.ToolType.BRUSH
            ...
            val importedCount = ImportedBrushRepository.all().size
            dialog = AlertDialog.Builder(this@MainActivity)
                .setTitle("Pincéis — ${BrushPresetRepository.all().size} presets  •  ${importedCount} importados")
                .setView(outer)
        ...
        fun importBrushQr(uri: Uri?) {
            if (uri == null) return
            try {
                val bitmap = contentResolver.openInputStream(uri)?.use { BitmapFactory.decodeStream(it) }
                if (bitmap == null) {
                    Toast.makeText(this@MainActivity, "Não foi possível abrir a imagem do QR.", Toast.LENGTH_LONG).show()
                    return
                }
                val result = com.animame.editor.BrushQrImporter.importBitmap(bitmap)
                bitmap.recycle()
                if (result == null) {
                    Toast.makeText(this@MainActivity, "QR de pincel não reconhecido.", Toast.LENGTH_LONG).show()
                    return
                }
                ImportedBrushRepository.add(this@MainActivity, result.settings)
                brushId = result.settings.id
                brushName = result.settings.name
                brushCategory = result.settings.category
                tool = EditorToolEngine.ToolType.BRUSH
                Toast.makeText(this@MainActivity, if (result.ipbz) "Pincel IPBZ importado e preservado." else "Pincel QR importado.", Toast.LENGTH_LONG).show()
                invalidate()
            } catch (e: Throwable) {
                Toast.makeText(this@MainActivity, "Falha ao importar pincel QR: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }

        fun adjustZoom(d: Float) { zoom = (zoom + d).coerceIn(.1f, 16f); invalidate() }
        fun resetViewport() { zoom = 1f; panX = 0f; panY = 0f; viewportRotation = 0f; invalidate() }
        fun insertFrame() { document.insertFrame(document.currentFrame); document.currentFrame = (document.currentFrame + 1).coerceAtMost(document.duration - 1); contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun duplicateFrame() { document.duplicateFrame(document.currentFrame); document.currentFrame = (document.currentFrame + 1).coerceAtMost(document.duration - 1); contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun deleteFrame() { if (document.duration <= 1) { clearCurrentFrame(); return }; document.deleteFrame(document.currentFrame); document.currentFrame = document.currentFrame.coerceIn(0, document.duration - 1); contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun previousFrame() { stopPlayback(); document.currentFrame = (document.currentFrame - 1).coerceAtLeast(0); contentVersion++; invalidate(); refreshTimeline() }
        fun nextFrame() { stopPlayback(); document.currentFrame = (document.currentFrame + 1).coerceAtMost(document.duration - 1); contentVersion++; invalidate(); refreshTimeline() }
        fun toggleOnion() { onionEnabled = !onionEnabled; contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun clearCurrentFrame() { document.activeLayer.frameAt(document.currentFrame)?.strokes?.clear(); contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun addLayer() { val layer = document.addLayer(); activeLayerId = layer.id; contentVersion++; rasterCache.invalidate(); invalidate(); refreshTimeline() }
        fun togglePlayback() { if (playing) stopPlayback() else startPlayback() }
        fun startPlayback() { if (document.duration <= 1) { Toast.makeText(this@MainActivity, "Adicione pelo menos 2 frames para reproduzir", Toast.LENGTH_SHORT).show(); return }; playing = true; playback.play(); refreshTimeline(); invalidate() }
        fun stopPlayback() { if (!playing) return; playing = false; playback.pause(); refreshTimeline(); invalidate() }
        ...
        fun refreshTimeline() {
            timelineLabel?.text = "Frame ${document.currentFrame + 1}/${document.duration}  •  ${document.activeLayer.name}  •  ${document.fps} FPS  •  ${if (playing) "Reproduzindo" else "Parado"}"
            frameStrip?.let { strip ->
                strip.removeAllViews()
                for (i in 0 until document.duration) {
                    strip.addView(TextView(this@MainActivity).apply {
                        text = "${i + 1}"
                        textSize = 12f
                        gravity = android.view.Gravity.CENTER
                        setTextColor(ThemeColorStore.TEXT)
                        setBackgroundColor(if (i == document.currentFrame) accent else if (hasContent(i)) ThemeColorStore.NAVY_800 else ThemeColorStore.NAVY_950)
                        setOnClickListener { stopPlayback(); document.currentFrame = i; contentVersion++; invalidate(); refreshTimeline() }
                    }, LinearLayout.LayoutParams(52, 50).apply { setMargins(3, 4, 3, 4) })
                }
            }
        }
